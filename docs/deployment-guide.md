...
**This project's collision database (§1-7, `docs/collision-database.md`) is verified against `ide0` only.** `keyman` computes the SOFTWARE ID differently depending on how the disk is presented to the guest kernel -- `ide0` (real ATA IDENTIFY data) vs. `scsi0`/`sata0`/`virtio-scsi-pci` (SCSI INQUIRY + VPD page 0x80, a different encoding -- see [license-internals.md §8](license-internals.md#8-arm32-keyman-on-virtio-scsi-a-platform-specific-investigation)). **A `serial=`/`model=` combo verified for `ide0` will not produce the same SOFTWARE ID on `scsi0`/`sata0`, and vice versa** -- pick one bus type and use collision-search results/tooling (`ros-serialgen`'s `-b`/`--bus` flag) matched to it.
